...
module.exports = function (context) {
    ...
    function parseDoc(commentNode) {
        ...
        try {
            ...
            var line = commentNode.loc.start.line + (ex.line | 0);
            var column = commentNode.loc.start.column + (ex.column | 0);
    ...
    function checkBasicSyntax(node, jsdocNode, jsdoc, lines, startLine, startColumn) {
        ...
        var checkParams = function (param) {
            ...
            if (jsdocParam
                && name
                && name !== jsdocParam
                && (params[name] || jsdocParams.length === node.params.length)
            ) {
            ...
            else if (!hasOverride && name && !params[name]) {
                context.report(
                    jsdocNode,
                    {line: startLine + 1, column: startColumn},
        ...
        node.params && node.params.forEach(function check(param, i) {
            var type = param.type;
            if (type === 'ArrayPattern' || type === 'ObjectPattern') {
                (param.elements || param.properties).forEach(check);
            }
            else if (type === 'Property' && param.value.type === 'ObjectPattern') {
     ...
     * @type {RegExp}
     */
    var IGNORE_PATTERN = /^\s*(?=eslint|istanbul|jshint|jslint|jscs)/;
