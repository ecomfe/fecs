    ...
    create: function (context) {
        ...
        function fill(writeExpr, variable, variables) {
            ...
            switch (writeExpr.type) {
                ...
                case 'MemberExpression':
                    ...
                    from = variables[expr.split('.').shift()];
        ...
        function validate(node) {
            ...
            scope.variables.forEach(function (variable, i) {
                ...
                if (name !== 'arguments' && variable.scope.type !== 'global') {
            ...
            refs && refs.forEach(function (ref, i) {
                var id = ref.identifier;
                var name = id.name;
                if (ref.writeExpr && variables[name]) {
            ...
            Object.keys(variables).forEach(function (name, i) {
                ...
                variable.node.references.some(function (ref) {
                    ...
                    if (writeExpr && !writeExpr.type.match(LITERAL_TYPE_PATTERN)) {
                        var start = writeExpr.loc.start;
                        if (sourceCode.getLines()[start.line - 1].slice(0, start.column).match(/(^|;)\s*export\s/)) {
                            return false;
                        }
