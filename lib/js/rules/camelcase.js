...
module.exports = function (context) {
    ...
    var regPrivate = /^_[a-z\d][a-zA-Z\d]*(?=[A-Z])[a-zA-Z\d]*$/;
    function isPrivate(name) {
        return regPrivate.test(name);
    }
    ...
    return {
        ...
        'Property': function (node) {
            ...
            if (ignoreWithQuotes && node.key.type === 'Literal' && !isPrivate(name)) {
                return;
            }

            if ((isPrivate(name) && !hasPrivateTag(node) || isDashed(name)) && !isIgnored(name)) {
