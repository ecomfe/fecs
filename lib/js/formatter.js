...
module.exports = function (options) {
    ...
    return through(
        ...
        function (file, enc, cb) {
            ...
            var esformatterConfig = util.getConfig('esformatter', options.lookup ? file.path : '');

            try {
                var contents = file.contents.toString();


                contents = options.safe === 'low'
                    ? fix(contents, jshintConfig)
                    : safeFix(contents, jshintConfig);

                contents = options.safe === 'high'
                    ? contents
                    : esformatter.format(jformatter.format(contents, jformatterConfig), esformatterConfig);
