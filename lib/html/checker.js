...
function checkScript(contents, path, cliOptions, pos, errors) {
    var code =  ''
        // 在页面中应该禁止的规则
        + '/* eslint-disable fecs-valid-jsdoc, fecs-eol-last */'
        // 缩进需要调整
        + '/* eslint fecs-indent: [2, "spaces", 4, ' + (pos.col - 1) + '] */'
        + '\n'
        + contents.trimRight();
...
function checkStyle(contents, path, cliOptions, pos, errors) {
    var code =  ''
        // 在页面中应该禁止的规则
        + '/* csshint-disable no-bom */'
        // 缩进需要调整
        + '/* csshint block-indent: ["    ", ' + (pos.col - 1) + '] */'
        + '\n'
        + contents.trimRight();
...
checker.check = function (contents, path, cliOptions) {
    ...
    if (!cliOptions.type && config['script-content']) {
        linters.script = function (contents, pos, element) {
            checkScript(contents, path, cliOptions, pos, errors);
        };
    }
    ...
    if (!cliOptions.type && config['style-content']) {
        ...
        var timer;
        linters.style = function (contents, pos, element) {
            promises.push(checkStyle(contents, path, cliOptions, pos, errors));
            clearTimeout(timer);
