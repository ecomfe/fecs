...
exports.getConfig = function (key, filepath, defaults, force) {
    ...
    if (!localRcloader || force) {
        var name = CONFIG_NAME;
        var loader = parseJSON;

        if (!findPath(CONFIG_NAME) && !!findPath(PACKAGE_NAME)) {
    ...
    return localRcloader.for(filepath)[key] || defaults;
};
...
function readConfigs(dir, out) {
    ...
    fs.readdirSync(dir).forEach(function (file) {
        ...
        if (match) {
            ...
            out[key] = parseJSON(path.join(dir, file));
        }
    });
}
