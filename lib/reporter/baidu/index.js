...
var baidu = {
    ...
    eslintRules: {
        ...
        'space-in-brackets': function (error) {
            var nodetype = error.origin.nodeType;
            return nodetype === 'MemberExpression' ? '011' : '012';
        },
        ...
        'new-cap': function (error) {
            return ~error.message.indexOf('only') ? '030' : '029';
        }
 ...
 */
exports.transform = function (file, json, options) {
    var success = true;
    var log = this.log;
    ...
    item.errors = errors.map(function (error) {
        ...
        if (typeof error.line === 'number') {
            info += ('line ' + util.fixWidth(error.line, 3));
            if (typeof error.column === 'number') {
                info += (', col ' + util.fixWidth(error.column, 3));
            }
            info += ', ';
        }
        ...
        log[level.warn ? 'warn' : 'error'](info);
